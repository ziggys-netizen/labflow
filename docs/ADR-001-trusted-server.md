...
**On the compliance point, honestly.** Cloud Functions for Firebase is not covered by Google's BAA, so Option A carries no compliance advantage to trade away. Two things are worth recording rather than acting on: Firestore *is* BAA-covered while **Firebase Authentication is not** — Identity Platform is, and it is an upgrade of the same service from the same console, should LabFlow ever need to make that claim. And PHI flowing through Vercel functions makes Vercel a sub-processor to disclose. Neither is a HIPAA question in The Gambia. Both are relevant to the cross-border assessment under s.37 of the Bill as published by the National Assembly (not verified against the enacted 2025 text) — see PRD v0.5 §12 — and both are cheaper to know now than to discover during an assessment.
